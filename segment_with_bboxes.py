#!/usr/bin/env python3
"""
Segment with Bounding Boxes - Fit oriented bounding boxes (OBBs) to mesh segments.

This tool:
1. Loads a mesh/point cloud file (PLY, OBJ, GLB, etc.)
2. Loads segmentation labels from NPY file
3. Fits an oriented bounding box (OBB) to each segment using PCA
4. Exports colored bounding boxes in multiple styles

Usage:
    python segment_with_bboxes.py --input model.glb --labels labels.npy --output output.ply
...
class OBBFitter:
    """Fits oriented bounding boxes to point clouds using PCA."""

    def __init__(self, min_points: int = 3, apply_filtering: bool = True):
        self.min_points = min_points
        self.apply_filtering = apply_filtering
        self.filter = PointCloudFilter()

    def fit_obb(self, points: np.ndarray, filter_points: bool = None) -> dict:
        """Fit oriented bounding box using PCA with optional point filtering.

        Args:
            points: Input point cloud (Nx3 array)
            filter_points: Override instance setting for filtering

        Returns dict with: center, dimensions, rotation, mesh, filtered_points, original_points
        """
        ...
        # Apply filtering to remove outliers and low-density points
        if should_filter:
            points = self.filter.filter_points(points)

        # Compute PCA for principal component orientation
        center = np.mean(points, axis=0)
        centered = points - center
        pca = PCA(n_components=3)
        pca.fit(centered)

        # Get PCA components and ensure right-handed coordinate system
        rotation = self._ensure_right_handed(pca.components_.T)

        # Transform to PCA space and get bounds
        ...
        # Adjust center to bbox center in world coordinates
        bbox_center_pca = (min_c + max_c) / 2
        center = center + bbox_center_pca @ rotation.T
    ...
    def _ensure_right_handed(self, rotation: np.ndarray) -> np.ndarray:
        ...
        # Check if determinant is negative (left-handed system)
...
class BBoxOverlapResolver:
    ...
    def compute_axis_overlap(self, bbox1: dict, bbox2: dict, axis: np.ndarray) -> dict:
        ...
        Returns:
            dict with:
                - overlap_amount: How much the boxes overlap (negative = gap)
                - overlap_ratio: Overlap as ratio of smaller projected extent
                - proj1, proj2: Half-extents projected onto axis
                - dist: Distance between centers along axis
        """
        ...
        # Project box extents onto axis (half-extents)
        proj1 = sum(abs(np.dot(rot1[:, i], axis)) * dims1[i] / 2 for i in range(3))
        proj2 = sum(abs(np.dot(rot2[:, i], axis)) * dims2[i] / 2 for i in range(3))

        # Combined extent and overlap
        combined = proj1 + proj2
        overlap_amount = combined - dist  # Positive = overlap, negative = gap

        # Overlap as ratio of smaller extent (full extent, not half)
    ...
    def boxes_overlap_excessive(self, bbox1: dict, bbox2: dict) -> Tuple[bool, List[dict]]:
        ...
        Unlike simple SAT which returns True for ANY overlap, this checks if
        overlap exceeds max_overlap_ratio on any separating axis.

        Returns:
            Tuple of (has_excessive_overlap, axis_info_list)
        """
        rot1 = bbox1['rotation']
        rot2 = bbox2['rotation']

        # Test 15 separating axes: 3 from each box + 9 cross products
        axes = []
        ...
        for source, idx, axis in axes:
            ...
            # Check if this axis shows separation (no overlap at all)
            if info['overlap_amount'] <= 0:
                # Found a separating axis - boxes don't overlap
                return False, []

            # Check if overlap is excessive
    ...
    def boxes_overlap(self, bbox1: dict, bbox2: dict) -> bool:
        ...
        Note: This returns True for ANY overlap. Use boxes_overlap_excessive()
        to check if overlap exceeds the allowed threshold.
        """
        center1 = bbox1['center']
        center2 = bbox2['center']
        dims1 = bbox1['dimensions']
    ...
    def compute_overlap_volume_approx(self, bbox1: dict, bbox2: dict) -> float:
        ...
        # Use mesh intersection for accurate overlap calculation
        try:
            mesh1 = bbox1.get('mesh')
            mesh2 = bbox2.get('mesh')
            if mesh1 is None or mesh2 is None:
                return 0.0

            # Approximate using center distance and dimensions
            center_dist = np.linalg.norm(bbox1['center'] - bbox2['center'])
            avg_extent = (np.mean(bbox1['dimensions']) + np.mean(bbox2['dimensions'])) / 2

            # Heuristic: closer centers = more overlap
    ...
    def compute_principal_axis_overlap(self, bbox1: dict, bbox2: dict) -> dict:
        ...
        This is more meaningful than SAT axes for determining how much
        boxes actually need to shrink to separate.

        Returns:
            dict with overlap info along center-to-center direction
        """
        center1 = bbox1['center']
        center2 = bbox2['center']
        dims1 = bbox1['dimensions']
        ...
        if dist < 1e-10:
            # Centers coincide - use arbitrary direction
            direction = np.array([1, 0, 0])
            dist = 0
        else:
            direction = direction / dist

        # Project box extents onto the center-to-center direction
        ...
        combined = proj1 + proj2
        overlap_amount = combined - dist  # Positive = overlap

        # Reference dimension: smaller projected extent (full, not half)
        min_extent = min(proj1, proj2) * 2
        if min_extent > 1e-10:
            overlap_ratio = overlap_amount / min_extent
        else:
    ...
    def is_containment_scenario(self, bbox1: dict, bbox2: dict,
        ...
        In containment scenarios:
        - Overlap ratio is very high (>>100%)
        - One box is much larger than the other
        - The center of the smaller box is within the extents of the larger box

        We should NOT shrink in containment scenarios because:
        - Both boxes correctly bound their respective clusters
        - The small part is attached to/within the larger structure
        - Shrinking would not reflect the actual geometry
        """
        vol1 = np.prod(bbox1['dimensions'])
        vol2 = np.prod(bbox2['dimensions'])

        # Volume ratio (larger / smaller)
        vol_ratio = max(vol1, vol2) / max(min(vol1, vol2), 1e-10)

        # If overlap is > 100% and volume ratio is > 5x, it's likely containment
        if principal_overlap['overlap_ratio'] > 1.0 and vol_ratio > 5.0:
            return True

        # Also check if one box's center is fully within the other box's extent
        # along the principal direction
        dist = principal_overlap['dist']
        proj1 = principal_overlap['proj1']
        proj2 = principal_overlap['proj2']

        # If center-to-center distance is less than the larger projection,
        # the smaller box is mostly contained
        if dist < max(proj1, proj2) * 0.8:
            # Small box is likely attached to or inside the large box
            if vol_ratio > 3.0:
                return True
    ...
    def compute_shrink_to_target(self, bbox1: dict, bbox2: dict,
        ...
        Strategy:
        - Skip containment scenarios (small box inside large box)
        - Compute overlap along center-to-center direction
        - Only shrink if that overlap exceeds threshold
        - Shrink along the box axes most aligned with overlap direction
        - Larger box takes more shrink burden (it has more "slack")

        Returns:
            Tuple of (modified_bbox1, modified_bbox2)
        """
        # Get overlap along center-to-center direction (most meaningful)
        principal_overlap = self.compute_principal_axis_overlap(bbox1, bbox2)

        # Skip containment scenarios - don't shrink when small box is inside/attached to large box
        if self.is_containment_scenario(bbox1, bbox2, principal_overlap):
            return bbox1, bbox2

        # Check if overlap exceeds threshold along principal direction
        if principal_overlap['overlap_ratio'] <= self.max_overlap_ratio:
            return bbox1, bbox2

        # Calculate how much overlap to remove
        ...
        # Volumes for determining shrink burden
        vol1 = np.prod(bbox1['dimensions'])
        vol2 = np.prod(bbox2['dimensions'])

        # LARGER box takes more of the shrink burden (it has more margin)
        # This prevents small parts from being crushed
        if vol1 >= vol2:
            shrink1_ratio = 0.7
            shrink2_ratio = 0.3
        else:
        ...
        # For bbox1: find most aligned local axis and shrink it
        rot1 = bbox1['rotation']
        ...
        if alignment1 > 0.1:
            ...
            # Cap shrink to 30% of the dimension to prevent collapse
            ...
            # Shift center away from other box
            shift1 = direction * (shrink_amount1 * 0.5)
            new_bbox1['center'] = bbox1['center'] - shift1

        # For bbox2: find most aligned local axis and shrink it
        rot2 = bbox2['rotation']
    ...
    def resolve_overlaps(self, bboxes: List[dict], segment_to_bbox: Dict[int, int],
        ...
        The algorithm:
        1. Find pairs with excessive overlap (> max_overlap_ratio)
        2. Shrink both boxes just enough to reduce overlap to the threshold
        3. Repeat until no excessive overlaps remain
        """
        if len(bboxes) <= 1:
            return bboxes
        ...
        for iteration in range(max_iterations):
            ...
            # Find pairs with excessive overlap
            ...
            # Process each pair with excessive overlap
        ...
        # Recreate meshes for modified boxes
        fitter = OBBFitter(apply_filtering=False)
...
class SegmentWithBBoxes:
    """Main class for segmenting meshes and fitting bounding boxes."""

    def __init__(self, apply_filtering: bool = True, resolve_overlaps: bool = True):
        self.fitter = OBBFitter(apply_filtering=apply_filtering)
    ...
    def process(self, mesh: trimesh.Trimesh, labels: np.ndarray,
        ...
        """Process mesh and fit bounding boxes to each segment."""
        ...
        if self.resolve_overlaps and self.overlap_resolver and len(bboxes) > 1:
            ...
            bboxes = self.overlap_resolver.resolve_overlaps(bboxes, segment_to_bbox)
    ...
    def export_results(self, bboxes: List[dict], segment_to_bbox: Dict[int, int],
        ...
        with open(info_path, 'w') as f:
            f.write("Segment Bounding Box Results\n")
            f.write("=" * 50 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Fit oriented bounding boxes to mesh segments",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python segment_with_bboxes.py -i model.glb -l labels.npy -o output.ply
  python segment_with_bboxes.py -i model.glb -l labels.npy -o output.ply --style all
  ...
  python segment_with_bboxes.py -i model.glb -l labels.npy -o output.ply --no-overlap-resolution
    ...
    processor = SegmentWithBBoxes(apply_filtering=apply_filtering, resolve_overlaps=resolve_overlaps)

    print(f"Options: filtering={'ON' if apply_filtering else 'OFF'}, overlap_resolution={'ON' if resolve_overlaps else 'OFF'}")
    ...
    print(f"\n{'=' * 50}")
    print(f"Fitted {len(bboxes)} bounding boxes")
